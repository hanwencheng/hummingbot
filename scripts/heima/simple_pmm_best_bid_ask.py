...
class SimplePMMConfig(BaseClientModel):
    ...
    bid_spread: Decimal = Field(0.001, client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Enter the bid order spread:"))
    ask_spread: Decimal = Field(0.001, client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Enter the ask order spread:"))
...
class SimplePMM(ScriptStrategyBase):
    """
    Extends the Configurable Simple PMM Example script to take into account the prices and amounts of the best bid and ask orders on the book.
    """
    ...
    def on_tick(self):
        if self.create_timestamp <= self.current_timestamp:
            ...
            proposal_adjusted_best_bid_ask: List[OrderCandidate] = self.adjust_proposal_for_best_bid_ask(proposal_adjusted)
            self.place_orders(proposal_adjusted_best_bid_ask)
    ...
    def adjust_proposal_for_best_bid_ask(self, proposal: List[OrderCandidate]) -> List[OrderCandidate]:
        best_bid = self.get_order_book_dict(self.config.exchange, self.config.trading_pair)["bids"][0]
        self.log_with_clock(logging.INFO, f"Best Bid: {best_bid}")
        proposal[0].price = Decimal(min(proposal[0].price, best_bid[0]))
        if proposal[0].price == best_bid[0]:
            proposal[0].amount = Decimal(min(proposal[0].amount, best_bid[1]))
            self.notify_hb_app_with_timestamp(f"Adjusted bid order to {proposal[0].price:.6f} price and {proposal[0].amount:.2f} amount")
        
        best_ask = self.get_order_book_dict(self.config.exchange, self.config.trading_pair)["asks"][0]
        self.log_with_clock(logging.INFO, f"Best Ask: {best_ask}")
        proposal[1].price = Decimal(max(proposal[1].price, best_ask[0]))
        if proposal[1].price == best_ask[0]:
            proposal[1].amount = Decimal(min(proposal[1].amount, best_ask[1]))
            self.notify_hb_app_with_timestamp(f"Adjusted ask order to {proposal[1].price:.6f} price and {proposal[1].amount:.2f} amount")
